...
import { startNegotiation, getNegotiation } from '../../lib/negotiateApi.js'
...
export function NegotiateModal({ item, onClose }) {
  ...
  const [step, setStep] = useState('form') // form | sending | negotiating | done | error
  ...
  function startPolling(id) {
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (data.status === 'done') {
          ...
          stopPolling()
          setError('The negotiation failed unexpectedly. Please try again.')
          setStep('error')
        }
      } catch {}
    }, POLL_INTERVAL_MS)
    ...
    timeoutRef.current = setTimeout(() => {
      stopPolling()
      setError('Negotiation timed out after 15 minutes.')
      setStep('error')
    }, TIMEOUT_MS)
  ...
  return (
    <div className="neg-overlay" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="neg-modal" role="dialog" aria-modal="true" aria-label={`Negotiate with ${company}`}>
        ...
        {step === 'sending' && (
          <div className="neg-modal__body neg-modal__body--center">
            ...
            <p className="neg-status-text">Sending opening message to {company}…</p>
          </div>
        )}

        {step === 'negotiating' && (
          <div className="neg-modal__body neg-modal__body--thread">
            <div className="neg-thread">
              {messages.length === 0 ? (
                <div className="neg-thread__empty">
                  <div className="neg-pulse-ring" />
                  <p className="neg-status-sub">Waiting for {company} to reply…</p>
                </div>
              ) : (
                messages.map((msg, i) => (
                  <div
            ...
            <div className="neg-thread__footer">
              <span className="neg-timer">{formatElapsed(elapsed)}</span>
              <span className="neg-hint">Waddle is negotiating on your behalf…</span>
            </div>
          </div>
        )}
        ...
        {step === 'done' && (
          <div className="neg-modal__body">
            <div className="neg-reply-badge">Negotiation complete</div>

            {messages.length > 0 && (
              <div className="neg-thread neg-thread--compact">
                {messages.map((msg, i) => (
                  <div
                    key={i}
                    className={`neg-bubble neg-bubble--${msg.role === 'agent' ? 'agent' : 'supplier'}`}
            ...
            {summary && (
              <div className="neg-summary">
                <p className="neg-summary__label">Summary</p>
                <p className="neg-summary__text">{summary}</p>
              </div>
            )}

            <button className="neg-btn neg-btn--primary" onClick={onClose}>Done</button>
          </div>
        )}
